        ...
        static void Part1(string filename) {
            using(StringReader reader = new StringReader(File.ReadAllText(filename))){
                ...
                for(int i = 1; i < maxHeights.Length; i += 1){
                    maxHeights[i] = new TreeHeights[line[0].Length];
                ...
                for(int i = 1; i < maxPos; i += 1){
                    for(int j = 1; j < maxPos; j += 1){
                        maxHeights[i][j].highestUp = Math.Max(maxHeights[i-1][j].highestUp, forest[i][j]);
                        maxHeights[maxPos - i][j].highestDown = Math.Max(maxHeights[maxPos-i+1][j].highestDown, forest[maxPos - i][j]);
                        maxHeights[j][i].highestLeft = Math.Max(maxHeights[j-1][i].highestUp, forest[j][i]);
                        maxHeights[j][maxPos - i].highestRight = Math.Max(maxHeights[j][maxPos-i+1].highestRight, forest[j][maxPos - i]);
                    }
                }
